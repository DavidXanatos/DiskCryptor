...
static int argon2_selftest_ctx (void)
{
   ...
   u8 hash[32], pwd[32], salt[16], sec[8], ad[12];
   ...
   static const u8 ref0d[32] =          /* Reference hash results */
   ...
   context.out = (u8*) hash;           /* Init Argon2 context */
   context.outlen = 32;
   context.pwd = (u8*) pwd;
   context.pwdlen = 32;
   context.salt = (u8*) salt;
   context.saltlen = 16;
   context.secret = (u8*) sec;
   context.secretlen = 8;
   context.ad = (u8*) ad;
   context.adlen = 12;
   context.t_cost = 3;
 ...
 * Calls argon2_hash().
 */
static int hashtest (u32 t, u32 m, u32 p, const char *pwd,
                     const char *salt, argon2_type type, const u8 *ref)
{
   u8 out[32];
